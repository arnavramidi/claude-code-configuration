...
Before writing anything, identify all gaps or assumptions in the user's idea that would affect how the feature is built. Ask these as a numbered list of focused questions. Cover things like:

- Target users and their technical level
- Data persistence (session-only, local file, database)
- External integrations or APIs needed
- UI/UX preferences (layout, platform, accessibility)
- Scope boundaries (what is explicitly out of scope)
- Any constraints (performance, security, cost)

Wait for the user to answer before proceeding to Step 5. Do not draft the spec until you have their responses.

## Step 5. Draft the spec content

Using the user's answers from Step 4, create a markdown spec document that Plan mode can use directly and save it in the _specs folder using the `feature_slug`. Use the exact structure as defined in the spec template file here: @_specs/template.md. Do not add technical implementation details such as code examples.
